...
const TEMPLATE_FOR_INDUSTRY = {
  ...
  hvac: "bold-trade",
  roofer: "bold-trade",
  contractor: "bold-trade",
  handyman: "fresh-local",
  painter: "fresh-local",
  landscaper: "fresh-local",
  detailer: "fresh-local",
  dentist: "clinical-calm",
  chiropractor: "clinical-calm",
  restaurant: "warm-table",
  salon: "warm-table",
  realestate: "fluid-glass",
  attorney: "modern-pro",
  accountant: "modern-pro",
  insurance: "modern-pro",
  other: "fluid-glass",
};
...
export function buildContent(g) {
  ...
  return {
    ...
    theme: {
      ...
      accent: g.theme?.accent || "#b4532a",
      ...(g.theme?.bg ? { bg: g.theme.bg } : {}),
      ...(g.theme?.fg ? { fg: g.theme.fg } : {}),
    },
    template: g.template || TEMPLATE_FOR_INDUSTRY[industry] || "fluid-glass",
  };
}
