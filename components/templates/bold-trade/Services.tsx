...
export default function Services() {
  ...
  return (
    <Section id="services" eyebrow="What we do" index="01" title="Services">
      <motion.div
        {...staggerParent()}
      ...
      >
        {services.map((s, i) => (
          ...
          >
            <p className="text-eyebrow text-accent">{String(i + 1).padStart(2, "0")}</p>
            <h3 className="text-display mt-5 text-xl uppercase transition-colors duration-300 group-hover:text-accent">
              {s.title}
            </h3>
            <p
